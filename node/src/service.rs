...
use appchain_gameland_runtime::{opaque::Block, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		appchain_gameland_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		appchain_gameland_runtime::native_version()
	}
}
...
pub fn create_extrinsic(
	...
	function: impl Into<appchain_gameland_runtime::Call>,
	nonce: Option<u32>,
) -> appchain_gameland_runtime::UncheckedExtrinsic {
	...
	let period = appchain_gameland_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: appchain_gameland_runtime::SignedExtra = (
		frame_system::CheckSpecVersion::<appchain_gameland_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<appchain_gameland_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<appchain_gameland_runtime::Runtime>::new(),
		frame_system::CheckEra::<appchain_gameland_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<appchain_gameland_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<appchain_gameland_runtime::Runtime>::new(),
		pallet_transaction_payment::ChargeTransactionPayment::<appchain_gameland_runtime::Runtime>::from(tip),
	);

	let raw_payload = appchain_gameland_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			appchain_gameland_runtime::VERSION.spec_version,
			appchain_gameland_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	appchain_gameland_runtime::UncheckedExtrinsic::new_signed(
		function.clone(),
		sp_runtime::AccountId32::from(sender.public()).into(),
		appchain_gameland_runtime::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
