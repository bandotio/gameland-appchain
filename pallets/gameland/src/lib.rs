...
pub mod pallet {
	...
	pub(super) type Nft_owner<T> = StorageValue<_, (ClassIdOf<T>, TokenIdOf<T>), ValueQuery>;//owner->token

    //checked
	#[pallet::storage]
	#[pallet::getter(fn nfts)]
	pub type Nfts<T> = StorageValue<_, NftInfo, ValueQuery>; //token->nft_info

	//checked
	#[pallet::storage]
	#[pallet::getter(fn nft_basic_status)]
	pub(super) type Nft_basic_status<T> = StorageValue<_, NftInfo, ValueQuery>; //owner->nft_info
	...
	pub(super) type Borrow_or_not<T> = StorageValue<_, (ClassIdOf<T>, TokenIdOf<T>), ValueQuery>; //token -> bool
	
	//checked
	#[pallet::storage]
	#[pallet::getter(fn all_nfts)]
	pub type All_nfts<T> = StorageValue<_, (ClassIdOf<T>, TokenIdOf<T>), ValueQuery>; //tokens

	//checked
	#[pallet::storage]
	#[pallet::getter(fn borrow_status)]
	pub(super) type Borrow_status<T> = StorageValue<_, Borrow_info>;//token -> borrow_info
