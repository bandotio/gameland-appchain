...
pub mod module {
	...
	pub type OwnersByToken<T: Config> = StorageDoubleMap<_, Twox64Concat, (T::ClassId, T::TokenId), Twox64Concat, T::AccountId, ()>;
...
impl<T: Config> Pallet<T> {
	...
	pub fn create_class(owner: &T::AccountId, metadata: Vec<u8>, data: T::ClassData) -> Result<T::ClassId, DispatchError> {
		let class_id = NextClassId::<T>::try_mutate(|id| -> Result<T::ClassId, DispatchError> {
			let current_id = *id;
			*id = id.checked_add(&One::one()).ok_or(Error::<T>::NoAvailableClassId)?;
			Ok(current_id)
		})?;
		let info = ClassInfo { metadata, total_issuance: Zero::zero(), owner: owner.clone(), data };
	...
	pub fn transfer(from: &T::AccountId, to: &T::AccountId, token: (T::ClassId, T::TokenId), quantity: T::TokenId) -> Result<bool, DispatchError>{
		//少了校验from是owner,因为转的不是单币所以不能用原来的remove方式
	...
	pub fn mint(owner: &T::AccountId, class_id: T::ClassId, metadata: Vec<u8>, data: T::TokenData, quantity: T::TokenId) -> Result<T::TokenId, DispatchError>{
		NextTokenId::<T>::try_mutate(class_id, |id| -> Result<T::TokenId, DispatchError> {
			...
			*id = id.checked_add(&One::one()).ok_or(Error::<T>::NoAvailableTokenId)?;
			...
			Classes::<T>::try_mutate(class_id, |class_info| -> DispatchResult {
				...
				info.total_issuance =
					info.total_issuance.checked_add(&quantity).ok_or(Error::<T>::NumOverflow)?;
				Ok(())
			})?;

			let token_info = TokenInfo { metadata, data, quantity };
